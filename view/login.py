...
class LoginGUI(object):
    def __init__(self, root):
        ...
        resizeBG = imageBG.resize((490,50), Image.Resampling.LANCZOS)
        imageIconUser = Image.open(os.path.abspath('view/images/icon-user.png'))
        resizeUser = imageIconUser.resize((25,25), Image.Resampling.LANCZOS)
        imageIconPassword = Image.open(os.path.abspath('view/images/icon-password.png'))
        resizePassword = imageIconPassword.resize((25,25), Image.Resampling.LANCZOS)
        imageIconLogo = Image.open(os.path.abspath('view/images/logo.png'))
        resizeLogo = imageIconLogo.resize((100,100), Image.Resampling.LANCZOS)
        ...
        resizeFormBG = imageBG.resize((500,500), Image.Resampling.LANCZOS)
    ...
    def checkLogin(self, username, password):
        ...
        for u, p in zip(user_list, password_list):
            if(username.strip() == u and password.strip() == p):
                flagLogin = 1
            #Chuyển đổi password nhập vào thành chuỗi băm -> so sánh chuỗi băm đó với thông số trong hệ thống
